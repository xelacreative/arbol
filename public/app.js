...
socket.on('state:sync', (state) => {
  ...
  // Si está finalizado, aplicar el modo correspondiente según el rol
  if (state.finalized) {
    if (isAdmin) {
      // El admin se queda en su panel y solo se aplica la clase CSS
      showAdminPanel();
...
/* ==========================================================================
MODO FINALIZADO
- Admin: se queda en su panel, solo se oculta el árbol con CSS (clase .finalized)
- Participante: va a la pantalla final
======================================================================== */
function enterFinalizedMode() {
  if (isAdmin) {
    // El admin NUNCA sale de su panel
    showAdminPanel();
    screens.adminPanel.classList.add('finalized');
  } else {
    showScreen('final');
...
function exitFinalizedMode() {
  screens.adminPanel.classList.remove('finalized');
  if (finalAmbientInterval) {
    clearInterval(finalAmbientInterval);
    finalAmbientInterval = null;
  }
}
...
/* ==========================================================================
PANTALLA FINAL — solo para participantes
======================================================================== */
